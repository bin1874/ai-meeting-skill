...
第一版优先支持 Codex 和 Claude Code。Qoder、OpenCode、Cursor、Gemini 和 Hermes 已作为实验 provider 注册，但默认不推荐、不自动启用；只有 `doctor` 显示可用并通过 smoke 验证后，才适合在真实会议中显式指定。其他 CLI Agent 工具通过 provider adapter 架构预留，只有确认其 CLI 会话和权限隔离能力后才启用。
